 ...
 * Each seglist is descending doubly linked list.
 * 
 */
...
static void* find_fit_in_segindex (size_t segindex, size_t asize) {
    ...
    if (finder_bp == NULL || GET_SIZE(HDRP(finder_bp)) < asize) return NULL;

    while (1) {
        if (SUCC_BLKP(finder_bp) == NULL)
            break;
        if (GET_SIZE(HDRP(SUCC_BLKP(finder_bp))) < asize)
            break;
...
static void push_in_seglist (void* bp) {
    ...
    // push in seglist, maintaining descending order
    void* finder_bp = *seglist;
    if (GET_SIZE(HDRP(bp)) > GET_SIZE(HDRP(finder_bp))) {
        *seglist = bp;
        PUT(PREDP(bp), NULL);
